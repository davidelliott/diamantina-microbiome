...
Landscape processes in many drylands are driven by the interplay between fluvial and Aeolian processes \citep{mctainsh1987desert,bullard2003aeolian,field2009toward,langford1989fluvial}. \citet{mctainsh1987desert} proposed a model of sediment recycling that provides an integrated interpretation of these processes in the landscape. Rivers feeding internal draining river basins such as Lake Chad and Lake Eyre transport vast amounts of fine sediments, feeding the large dust source regions out of Africa, China and Australia. By linking river systems and wind erosion, McTainsh proposed looking at arid geomorphology as a system of sediment recycling operating on a massive scale. Microbiota have not generally been considered as a component of this fluvial - aeolian interaction, however this is perhaps an oversight as they are well recognised for their stabilising influence in drylands generally through the formation of biocrusts.

The extent to which “microscopic ecosystem engineers” contribute to interacting fluvial-aeolian geomorphic processes, and if they in turn rely on the geomorphology to enable their survival is explored in this paper. We investigate the microbial ecology of a dynamic dryland ecosystem in central Australia comprising a 25 km$^{2}$ claypan in the Lake Eyre basin bounded by sand dunes and ephemeral rivers. Fluvial-aeolian interactions similar to those presently occurring here are thought to have been occurring at least since the last glacial maximum about 10 kBp \citep{bullard2003aeolian}. Before that structural controls for internally draining rivers were set up 20 –- 24 Ma \citep{simon1996evolution}, establishing an enclosed geological system of fluvial-aeolian sediment movement that has persisted in some form to the present day. 
...
Three distinct surface layers were observed in the claypan:
\begin{enumerate}
...
DNA was extracted in the field within 24 hours of sample collection using a Mobio Powersoil DNA extraction kit according to the manufacturers instructions except for increasing the soil amount slightly to 0.4 g per sample as described previously \citet{elliott2014niche}. DNA sequencing targeted the prokaryotic 16S rRNA gene and was performed by the Centre for Genomic Research (CGR) NERC Facility at the University of Liverpool, using an Illumina MiSeq in a Paired end approach. Resultant FastQ files were processed using QIIME\footnote{a bit more detail will be provided} \citep{caporaso2010qiime} for clustering, OTU table generation, and taxonomy assignment. Clustering used the xx method and operational taxonomic units (OTUs) were defined at 97 \% similarity which typically approximates to species-level differences (REF). These results were analysed using R and the phyloseq package \citep{mcmurdie2013phyloseq} for R.
...
A total of \Sexpr{length(unique(tax_table(bac)[,"Phylum"]))} bacterial phyla and \Sexpr{length(unique(tax_table(arc)[,"Phylum"]))} archael phyla were identified. Frequencies of the most abundant phyla are shown in Figure \ref{fig:waterp}. It can be seen that water channel samples are dissimilar to the soil-based samples at the phylum level although all abundant phyla are represented in both soil and water. Some phyla differed in abundance between the dune flank soil and airborne dust liberated from the same soil by wind tunnel treatment, for example \textit{Cyanobacteria} (more abundant in soil) and \textit{Firmicutes} (more abundant in dust).
...
<<nmds_out, echo=FALSE, fig.height=4, fig.cap='NMDS visualisation of prokaryotic community structure in soil (sand dunes, claypan, and nebkha) and airborne dust (from sand dunes). Water channel data are not shown on this plot because their extreme dissimilarity to other samples would require plotting at a different scale which obscures the patterns of interest between the other samples. \\label{fig:nmds}', cache=TRUE>>=
...
\begin{sidewaystable}
  \centering
    \caption{Summary of hypothesised dispersal strategies for biocrust microbes and their possible geomorphic effects. Example taxa for each mode of dispersal are given based on results from this study. Further candidate taxa can be identified from supplementary data ...}%
