...
#all_otutab <- read_delim("data/all.otutab.txt", "\t")
#all_otutab <- as.data.frame(all_otutab)
#row.names(all_otutab) <- all_otutab[,1]
#all_otutab <- all_otutab[,-1]
#otutab <- otu_table(all_otutab,taxa_are_rows = T)
#sample_names(otutab) <- gsub(pattern = "duneflank",replacement = "dune_flank",x = sample_names(otutab))

biomin <- import_biom(BIOMfilename = "data/all.otutab.biom")
# sample_names(biomin) <- gsub(pattern = "duneflank",replacement = "dune_flank",x = sample_names(biomin))
# greengenes taxonomy assigned using RDP
# taxin <- read_delim("data/uclust_assigned_taxonomy/all.otus_tax_assignments.txt", trim_ws = TRUE, delim = "\t", escape_double = FALSE, col_names = FALSE)
# SILVA taxonomy assigned using uclust
...
# processing for taxonomy if using RDP/greengenes
#### taxin <- as.data.frame(taxin)
#### row.names(taxin) <- gsub(pattern = ";.*",replacement = "",x = taxin$X1)

#### tax <- str_split_fixed(string = taxin$X2, pattern = ";",n = 7)
#### row.names(tax) <- gsub(pattern = ";.*",replacement = "",x = taxin$X1)
#### colnames(tax) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
#### tax <- gsub(pattern=" ?[a-z]__",replacement="",x = tax)
####

# processing for taxonomy if using UCLUST/SILVA
taxin <- as.data.frame(taxin)
row.names(taxin) <- gsub(pattern = ";.*",replacement = "",x = taxin$X1)
