...
OTU differential abundance analysis indicated that \Sexpr{deseq_n_diff_air} OTUs (\Sexpr{round(deseq_sum_diff_air*-1)} \% of reads) were  more abundant in air samples and \Sexpr{deseq_n_diff_earth} OTUs (\Sexpr{round(deseq_sum_diff_earth)} \% of reads) were more abundant in soil samples (adjusted p \textless 0.05). Details for a subset of these OTUs are shown in Tables \ref{tab:deseq1} and \ref{tab:deseq2} (extended results provided in Table S4). Results from these analyses were used to colour code Figures \ref{fig:waterp} and \ref{fig:waterc} to indicate whether taxa easily become airborne under moderate windy conditions or not. It can be seen from these figures that some phyla and particular cyanobacterial genera differed in abundance between the dune flank soil and airborne dust liberated from the same soil by wind tunnel treatment. For example phylum \textit{Cyanobacteria} was more abundant in soil (resist becoming airborne) and phylum \textit{Firmicutes} was more abundant in dust (easily become airborne). Detail of cyanobacterial distribution given in Figure \ref{fig:waterc} shows that the common cyanobacteria in the biocrust are from the genera \textit{Microcoleus} and \textit{Tychonema}. Most OTUs in these genera were more common in soil compared to airborne dust but some were classified as intermediate which means no significant difference in abundance was found between soil and airborne dust. Most of these OTUs were also found in river channels but OTUs belonging to \textit{Microcoleus} group SAG 1449-1a were not found in channels. \footnote{this needs revising to reflect latest results}
