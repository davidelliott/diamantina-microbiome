...
\begin{keypoints}
\item Microbial community of dry ephemeral lake bed and linked geomorphic features characterised
\item Wind preferentially mobilises specific taxa from dryland biological soil crusts (biocrusts)
\item Microbial dispersal mechanisms hypothesised to have variety of geomorphic implications
\end{keypoints}
...
write.csv(phyla.df[ ,c(rank_names(bac)[1:2],sample_names(ab.rel.phylum.merged))],"Elliott_Diamantina_Microbiome_Table_S1[phyla].csv", row.names = FALSE)
...
write.csv(cyano.df[ ,c(rank_names(bac)[1:6],sample_names(cyano.rel.g.merged))],"Elliott_Diamantina_Microbiome_Table_S2[cyanobacteria].csv", row.names = FALSE)
...
pdf("Elliott_Diamantina_Microbiome_Figure_S1[nmds].pdf")
print(NMDS.all)
dev.off()
...
bac.rel.merged.air <- merge_samples(subset_samples(bac.rel,element=="air"),"zone")
sample_names(bac.rel.merged.air) <- "dune flank air"
bac.rel.merged <- merge_phyloseq(bac.rel.merged,bac.rel.merged.air)
...
# save to file
write.csv(sigtab.subset,"Elliott_Diamantina_Microbiome_Table_S3[wind].csv")
...
The Chao1 species richness estimation (Figure \ref{fig:community}) indicated in excess of 10,000 species present in each sample, without striking differences with respect to sample type. 
...
# merge otu differential abundance results from sigtab.subset into otus.df for supplementary data

#length(row.names(sigtab))
#length(row.names(otus.df))

write.csv(otus.df[,c(rank_names(bac)[1:7],sample_names(bac.rel.merged),"sig")],"Elliott_Diamantina_Microbiome_Table_S4[otus].csv")
