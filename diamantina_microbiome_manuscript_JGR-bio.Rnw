...
# ordinate using phyloseq for stepwise model fitting
ord.step <- ordinate(ab.earth, "CCA") # this takes a long time
plot(ord.step, display="sites")

# get map out of phyloseq object
map.df3 <- data.frame(sample_data(ab.earth.rel))
...
(step.result <- ordistep(object=mod0,scope=formula(mod1),direction="forward"))
step.result$call
step.result$anova

# Plot the automatically selected model

levels(map.df3$zone)

cols <- c("green4", "mediumblue", "red2", "black")
plot(step.result,type = "none")
points(step.result,pch=as.numeric(map.df3$zone_code), col=cols[as.numeric(map.df3$zone_code)])
...
The dune and dune flank sediments are very similar in terms of particle-size distribution.  They are dominated by very fine to fine sands (46-48 pc) with the modal particle-size in the range 185-270 um.  The dune sediments are slightly coarser overall containing a higher percentage of medium and coarse sands and lower percentages of silt and clay than the dune flank (Table \ref{tab:partsize}).  The nebkha are also dominated by very fine to fine sands (60pc; mode 169-185 um) and the sediments are well-sorted.  The pan sites are less well sorted than any of the sites dominated by wind-blown material.  The pan sites include the highest proportion of clay (c. 10 pc) and silts (55 pc total) and lowest percentage of sand-sized material (<35 pc).
