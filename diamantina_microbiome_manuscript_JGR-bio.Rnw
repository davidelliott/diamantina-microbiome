...
water_otus_plot <- plot_bar(ab.rel.ea.p,x = "zone",fill = "source")
...
wind_otus_plot <- plot_bar(ab.rel.ea.p,x = "zone",fill = "behaviour")
...
otus.df.out <- otus.df[1:10,c(rank_names(ab)[c(2,6)],sample_names(ab.rel.merged),"sig")]
...
write.csv(otus.df[,c(rank_names(ab)[1:7],sample_names(ab.rel.merged),"sig")],"Elliott_Diamantina_Microbiome_Table_S4[otus].csv")
