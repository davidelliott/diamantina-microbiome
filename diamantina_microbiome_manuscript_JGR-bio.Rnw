%% 11/23/2015
% Note from DE:
% The % character is used to add comments - they do not appear in rendering.
% this version is from git commit 2018-07-21
% 
% * <d.r.elliott@derby.ac.uk> 2018-09-03T20:27:44.383Z:
% 
% Thanks for all your contributions. I am now merging all this to create a new version. Please no more edits in this system or I might miss them.
% 
% ^.
% if suggesting to add references please find them on google scholar and use the cite option 
% to obtain the bibtex entry - then paste that into the references.bib file.
%
% if you don't want to use this system please just supply your comments over email, no problem.
%
% the word count can be found using the toolbar at top. Currently it is 7,806
% AGU publication element is 500 words or 1 figure/table 
% - see https://publications.agu.org/author-resource-center/text-requirements/
% We therefore have 16 + 8 + 5 = 29 publication elements
% limit is meant to be 25 so some content needs to be reduced
% please consider this and give suggestions
%
% my suggestion is to
% - eliminate the map (figure 2), replace with supplementary gpx or kmz 
% * <j.e.bullard@lboro.ac.uk> 2018-07-31T11:33:17.042Z:
% 
% I think the map is useful - particularly the second panel as it shows the sampling strategy and geomorphic units.  Not convinced we need panel (a).  We can find a Landsat image of the area which would not have any copyright problems and would be sufficient resolution.
% I'm not 100% convinced we need figure 1 - I think we could explain what it shows in fewer than 500 words, and also we deal with this better in Table 5.  We could combine Fig 1 with Fig 2 - so just take the 3 photographs from Fig 1 (and possibly the nebkha photo from Fig 3) and arrange them in a row, add on the cycling arrows.  Put that row of 4 + arrows below panel b from Figure 2.  Loose the text from Fig 1.
% I think we can do without Figure 3 and also Figure 4.   If we did that, then we've straightaway lost 3 figures (or 3 PUs).
% 
% ^.
% (if keeping the map I think there are copyright issues that need to be overcome)
% - combine particle size table 1 into Figure 5 (convert to plot panels)
% * <j.e.bullard@lboro.ac.uk> 2018-07-31T11:40:27.410Z:
% 
% Yes, obvious move.  We could just add 3 columns to the particle size table, one for N +/- std error, one for C and one for pH.
% That loses us another PU.
% 
% ^.
% - eliminate figure 4 (sampling photos) or one of the latter tables
%
% please let me know your thoughts.
% * <j.e.bullard@lboro.ac.uk> 2018-07-31T11:42:27.019Z:
% 
% Another possibility is to combine Figure 7 and Figure 8. I know one is bacterial phyla and one is cyanobacteria genera, but the formats, legend and figure captions are virtually the same.  Have 4 panels a-d where a, b are bacterial phyla and cyanobacteria genera by source, and then c and d are the same but by behaviour.  It would make a full page but make a coherent figure.
% 
% ^.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\affiliation{4}{TBC.}
% * <j.e.bullard@lboro.ac.uk> 2018-07-31T11:47:17.442Z:
% 
% If no one has heard from Helene (I certainly haven't) and she has not contributed anything to the writing of the paper I would suggest you remove her from the author list and add her in to the acknowledgements instead as providing assistance in the field.   I don't know about yours but our authorship policy requires all authors to contribute to the actual writing of the paper.  Might sound harsh, but that would be my suggestion unless she has worked on the manuscript.
% 
% ^ <d.r.elliott@derby.ac.uk> 2018-09-03T13:14:42.179Z:
% 
% Helene and Grant helped a lot in the field - physically and intellectually, I wanted to acknowledge this. It could be done in acknowledgements section I agree. I will change it at the end depending on input received and any other comments from the others.
%
% ^.
...
\item Wind preferentially mobilises specific microbes from soil biocrusts
...
% * <j.e.bullard@lboro.ac.uk> 2018-07-31T11:50:26.840Z:
% 
% Need to make sure we mention dust - because not in title or key points (which is fine) but we do need it in the abstract when it gets written
% 
% ^.
...
bac <- import_biom("data/otus/Elliott_Diamantina_Microbiome_Data_S1[otu-tax].biom", parseFunction = parse_taxonomy_greengenes)
...
# ord2.ab <- ordinate(ab.earthair, "RDA", "bray")
# RDA.ab <- plot_ordination(ab.earthair, ord2.ab, type = "sites", color = "zone", shape="element") + geom_point(size=5,alpha=0.7) + theme(aspect.ratio=1) + theme(legend.position="right")
# RDA.ab$layers <- RDA.ab$layers[2]
# The RDA takes a very long time to calculate, so NMDS is preferred. Had originally used NMDS anyway but considered changing ro RDA for simplicity of using same approach as model building below, however the NMDS works much better.
...
dune.rel <- subset_samples(ab.rel,element=="air")
...
ab.rel.ea.p.nebkha <- subset_samples(ab.rel.ea.p,zone=="nebkha")
ab.rel.ea.p.nebkha.airborne <- prune_taxa(as.logical(tax_table(ab.rel.ea.p.nebkha)[,1]=="easily becomes airborne") , ab.rel.ea.p.nebkha)
pc_nebkha_easily_airborne <- 100*(sum(taxa_sums(ab.rel.ea.p.nebkha.airborne)) / sum(taxa_sums(ab.rel.ea.p.nebkha)) )
...
Sampled areas were 10x10 cm and were selected to represent the diversity of biocrust in the area. The sampled depth of approximately 5 mm was dictated by natural coherence of the biocrust. The area was cut out with a sterile scalpel and the crust was lifted off the unconsolidated soil beneath. In areas where differing crust types were noticeable the sampled areas were selected to have both types where applicable.
...
A wind tunnel was run on dune flanks within 3 m of where the soil crust sampling zone. The wind tunnel was a duct-type design equipped with a sediment collection trap in which sterile Whatman xx\footnote{need filter paper type} filter papers were fitted. Further details of design and operation have previously been described by \citet{strong2016development}\footnote{should provide a bit more detail here}. The wind tunnel was run on dune flanks because they are relatively flat, and being naturally eroded by saltation impact from the dune sand above during windy conditions they likely act as an inoculum source to surrounding areas more so than the higher dune areas. At each site the wind tunnel was run three times on adjacent areas for 5 minutes at 10 m/s for 5 minutes\footnote{Helene collected data to determine the exact run conditions}. This is a moderate treatment intended to be realistic and to collect only the easily mobilised particles / organisms. 
...
DNA was extracted in the field within 24 hours of sample collection using a Mobio Powersoil DNA extraction kit according to the manufacturers instructions except for increasing the soil amount slightly to 0.4 g per sample as described previously \citet{elliott2014niche}. DNA sequencing targeted the prokaryotic 16S rRNA gene v4 region and was performed by the Centre for Genomic Research (CGR) NERC Facility at the University of Liverpool, using an Illumina MiSeq in a Paired end approach (2x250 using v2 reagents). The primer design, PCR, and barcoding approach followed the method of \citet{caporaso2010global} using primers 515F and 806R (15 cycles) for amplification of v4 region, then a second nested PCR (15 cycles) to incorporate illumina adapter sequences and barcodes. Barcodes were as described in the Illumina Nextera protocol.

DNA sequence data from this research has been deposited in the NCBI Sequence Read Archive and is available via project accession SRP150526.
% We used our standard v4 primers for your previous project, as per the attached protocol, but with one change.  We performed 15 + 15 cycles for first and second round PCRs on the basis of your testing prior to sending us the samples.  Sequencing was 2x250 on the MiSeq using v2 reagents.  I’ve attached traces of the amplicons to this email for you.


\subsection{Bioinformatics}
FastQ files from DNA sequencing were processed using QIIME \citep{caporaso2010qiime} with default settings. Clustering used the UCLUST method \citep{edgar2010search} and open-reference OTU picking using the Greengenes 16S rRNA reference database \citep{desantis2006greengenes}. OTUs were defined at 97 \% similarity which typically approximates to species-level differences \citep{stackebrandt1994taxonomic}. \footnote{DE to add more detail}

\subsection{Statistical analyses}
OTU tables with assigned taxonomy from the bioinformatics pipeline were analysed using R \citep{R2017} with the phyloseq \citep{mcmurdie2013phyloseq} and vegan packages \citep{oksanen2017vegan}. Sequences identified as originating from chloroplasts or  mitochondria were removed. Microbial community structure was visualised with respect to sample source using nonmetric multidimensional scaling (NMDS) based on Bray-Curtis dissimilarity calculated from relative abundance of OTUs. In the resultant NMDS plots showing the first 2 dimensions of the multidimensional space, samples with similar community composition are placed closer together \citep{kruskal1964multidimensional}. Possible correlation of measured variables with microbial community structure was evaluated by stepwise model building to select explanatory variables in a redundancy analysis (RDA), and using ANOVA`' to test significance of selected variables to the model. All measured variables were used in this process including pH, C, N, and a range of particle size parameters (variables are available in supplementary table S1). Alpha diversity was evaluated by calculating the Chao1 richness estimate for each sample.
...
\subsection{Soil properties}
Soils and sediments all contained low concentrations of carbon and nitrogen (Figure \ref{fig:chem}). Dune and dune flank sites contained approximately double the amount of carbon (approximately 0.35 \% w/w) and nitrogen (approximately 0.055 \% w/w) compared to nebkha and pan sites (approximately 0.20 \% and 0.35 \% respectively). The pH of dune and dune flank sites were similar at pH 6.5 whereas pan and nebkha sites were around pH 7.0.


<<chem_out, echo=FALSE,fig.height=2,fig.cap='Soil properties in and around Lake Constance claypan at Diamantina National Park, Australia. a. Mean total carbon and nitrogen (percent w/w), pH, and particle mass median diameter (um); b. particle size distribution. Error bars represent the standard error of the mean for each soil type (n = 5). Sample coding for soil types: D sand dune, DF dune flank, N nebkha, P pan surface. \\label{fig:chem}', warning=FALSE, cache=TRUE>>=
# melt for plotting
...
# make the plot 
print(ggplot(chemistry.melt, aes(x=zone_code, y=value, shape=element, colour=zone)) + stat_summary(fun.data = "mean_se", size=1) + facet_wrap(~variable,scales = "free_y",nrow=1,labeller = label_parsed) + labs(x = "zone") + theme(aspect.ratio=1,legend.position="none") + colScale )
...
# make the plot 
print(ggplot(ps.melt, aes(x=zone_code, y=value, shape=element, colour=zone)) + stat_summary(fun.data = "mean_se", size=1) + facet_wrap(~variable,scales = "fixed",nrow=1,labeller = label_parsed) + labs(x = "zone", y = "%") + theme(aspect.ratio=1,legend.position="none") + colScale )

@


Dune and dune flank sediments had similar particle-size distribution.  They are dominated by very fine to fine sands (46-48 \%) with the modal particle-size in the range 185-270 $\mu$m.  The dune sediments are slightly coarser overall containing a higher percentage of medium and coarse sands and lower percentages of silt and clay than the dune flank (Figure \ref{fig:chem}).  Nebkha sediments are also dominated by very fine to fine sands (60 \%; mode 169-185 $\mu$m) and are well-sorted.  Pan sediments are less well sorted than any of the sites dominated by wind-blown material.  The pan sites include the highest proportion of clay (c. 10 \%) and silts (55 \% total) and lowest percentage of sand-sized material ($<$35 \%).

\subsection{DNA sequencing and general microbial community composition}
DNA was successfully extracted and sequenced from 30 samples, yielding 18.5 million reads. After pairing reads and quality control 7.7 million bacterial reads and 0.1 million archaeal reads remained. 

There were 80701 bacterial OTUs and 555 archael OTUs detected (OTUs defined by 97 \% similarity). This analysis was planned to target bacteria, and as archaea make up only 0.8 \% of sequences bacteria are the main focus of this paper, however archaeal reads were retained for the analyses. The Chao1 richness estimation (Figure \ref{fig:community}) indicated in excess of 10,000 OTUs present in each sample, without striking differences with respect to sample type. 

Initial visualisation of microbial community structure by NMDS (Figure S1) showed that the water samples differed extremely compared to all other samples, and this dominated all other inter-sample differences. The water samples were therefore excluded from the NMDS to reveal the community stucutre relationships between the soil and airborne dust samples as shown in Figure \ref{fig:community}. The dune community differs from the dune flank community, and the airborne dust community derived from the dune flank differs from the dune flank community. Notably the dust community has a structure approximately intermediate between the dune and nebkha communities.
...
@
\caption{ Relative abundance of cyanobacterial genera. Colouring indicates whether individual OTUs (97 pc similarity) contributing to the genus abundance were found in the water channels (top), and whether they were significantly more abundant in soil or wind tunnel collectors at the dune flank sites (bottom). Unidentified OTUs were excluded. Only the 5 most abundant identified cyanobacterial genera are shown for clarity (full data provided in Table S3). Source designation is not indicative of the ultimate origin of OTUs.}\label{fig:waterc}
\end{center}
\end{figure}

\subsection{Correlation between measured variables and microbial community structure}
A stepwise model building approach was used to identify measured variables which may explain microbial community structure. Only the particle size d50 (mass median diameter) had a significant correlation with microbial community structure (p=0.005). The mass median diameter was negatively correlated with axis RDA1 in the resultant model (Supplementary Figure S2), which indicates the smallest particle diameters were clearly associated with pan samples but other sample types did not clearly partition in relation to d50 / RDA1. 

\subsection{Dispersal of microbes by fluvial and aeolian processes}
Of the 20196 OTUs found in the claypan, 2640 were also found in water channels, 10226 were found in airborne dust from the dune flank, and 2020 were found in both. The dune and channel samples had 3838 OTUs in common. Whilst many soil OTUs were not detected in the air or water samples, most of the OTUs found exclusively in soil were rare, collectively accounting for only 0.3 \% of reads in the soil samples. 

OTU differential abundance analysis indicated that 648 OTUs (25 \% of reads) were  more abundant in air samples and 550 OTUs (25 \% of reads) were more abundant in soil samples (adjusted p \textless 0.05). Details for a subset of these OTUs are shown in Tables \ref{tab:deseq1} and \ref{tab:deseq2} (extended results provided in Table S4). Results from these analyses were used to colour code Figures \ref{fig:waterp} and \ref{fig:waterc} to indicate whether taxa easily become airborne under windy conditions or not. It can be seen from these figures that some phyla and particular cyanobacterial genera differed in abundance between the dune flank soil and airborne dust liberated from the same soil by wind tunnel treatment. For example phylum \textit{Cyanobacteria} was more abundant in soil (resist becoming airborne) and phylum \textit{Firmicutes} was more abundant in dust (easily become airborne). Detail of cyanobacterial distribution given in Figure \ref{fig:waterc} shows that \textit{Phormidium} and \textit{Microcoleus} species were more common in soil compared to airborne dust, whereas \textit{Leptolyngbya} species were more common in the soil compared to dust. \textit{Nodularia} species and \textit{Synechococcus} species were more common in water channels compared to soils, whilst the other genera mentioned were more common in soils. 


<<deseq_out1, echo=FALSE, cache=FALSE>>=
kable(outtab,digits=2,row.names=F, caption = "OTUs which are significantly more abundant in soil compared to dust blown from the soil. This is a subset of the differential abundance analysis output, sorted by magnitude of difference in abundance. The padj column indicates the probability of the observed distribution including adjustment for multiple testing. OTU IDs and extended results are provided in Table S4. \\label{tab:deseq1}")
@


<<deseq_out2, echo=FALSE, cache=FALSE>>=
kable(outtab2,row.names=F, digits=2,caption = "OTUs which are significantly more abundant in dust blown from the soil compared to the whole soil. This is a subset of the differential abundance analysis output, sorted by magnitude of difference in abundance. OTU IDs and extended results are provided in Table S4. \\label{tab:deseq2}")
@
...
\acknowledgments
This project was funded by NERC grant ref NE/K011464/1.
