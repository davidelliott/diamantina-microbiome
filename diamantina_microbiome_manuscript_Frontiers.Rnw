...
\def\Authors{David R. Elliott\,$^{1,*}$, Andrew D. Thomas\,$^{2}$, Craig L. Strong\,$^{3}$, Helene A. Aubault\,$^{4}$, Grant H. McTainsh\,$^{5}$ and Johanna Bullard\,$^{6}$}
...
$^{3}$The Fenner School of Environment \& Society, ANU College of Medicine, Biology \& Environment, The Australian National University, Canberra   ACT 2601,  Australia \\
$^{4}$TBC\\
...
Our main hypothesis is that wind differentially mobilises certain microbes from biocrusts, and that other microbes resist wind erosion. Related to this, we also expected to find biocrust organisms in watercourses.
