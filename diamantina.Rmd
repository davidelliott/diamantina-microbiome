...
```{r rich, echo=FALSE,fig.width=4,fig.height=3,fig.cap='Chao1 richness estimate\\label{fig:rich}', cache=FALSE}
...
```{r nmds, echo=FALSE,fig.height=6,fig.cap='NMDS visualisation of bacterial community structure\\label{fig:nmds}', cache=FALSE}
...
Check for channel bacteria on the pan (expected) and on the dune (should be less if hypothesis 1 is correct)
```{r hyp1, echo=FALSE,fig.height=6,fig.cap='Shared taxa between channel and pan (left), and between channel and dune (right). \\label{fig:shared}', cache=FALSE}

channel <- subset_samples(bac.rel,zone=="channel")
dune <- subset_samples(bac.rel,zone=="dune")
...
channel_dune.shared <- plot_bar(channel_dune,x = "zone",fill = "Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack") + scale_y_continuous(name="relative abundance (%)") + scale_fill_manual("Phylum", values = allphyla.col) + scale_colour_manual("Phylum", values = allphyla.col)


grid.newpage()
pushViewport(viewport(layout=gl))
print(channel_pan.shared,vp=vp.1)
print(channel_dune.shared,vp=vp.2)

```

Check for windswept microbes from the dune flank in all other samples.

```{r hyp2, eval=FALSE}
# This doesn't work all that well. DISABLED
airborne <- subset_samples(bac.rel,element=="air")
airborne <- prune_taxa(taxa_sums(airborne)>0,airborne)

bac.airborne.subset <- subset_samples(bac.rel,element!="air")
bac.airborne.subset <- prune_taxa(taxa_names(airborne), bac.airborne.subset)
bac.airborne.subset <- prune_taxa(taxa_sums(bac.airborne.subset)>0,bac.airborne.subset)
otu_table(bac.airborne.subset) <- otu_table(bac.airborne.subset)/5

plot_bar(bac.airborne.subset,x = "zone",fill = "Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack") + scale_y_continuous(name="relative abundance (%)") 

+ scale_fill_manual("Phylum", values = allphyla.col) + scale_colour_manual("Phylum", values = allphyla.col)
```

.

```{r hyp2v2, echo=FALSE,fig.height=6,fig.cap='OTU co-occurrence in soil samples, water samples, and airborne dust. \\label{fig:source}', cache=FALSE}
