...
# Identify top phyla and cyano genera - so they can be selectively plotted
top.phyla = names(sort(tapply(taxa_sums(bac.rel), tax_table(bac.rel)[, "Phylum"], sum), TRUE)[1:10])
top.cyano.genus = names(sort(tapply(taxa_sums(cyano.rel), tax_table(cyano.rel)[, "Genus"], sum), TRUE)[1:4])
# To simplify plots
...
# simplify data (for testing only)
bac.rel.ea <- prune_taxa(taxa_sums(bac.rel.merged) > 0.1, bac.rel.merged) 
# can prob do find/replace bac.rel.ea replace with bac.rel.ea

bac.rel.ea <- bac.rel
bac.rel.ea <- subset_samples(bac.rel.ea,element!="air")
otu_table(bac.rel.ea) <- otu_table(bac.rel.ea)/5

# select only the more abundant phyla
...
```{r chem_out, echo=FALSE,fig.height=3,fig.cap='Soil properties\\label{fig:chem}', warning=FALSE, cache=TRUE}
# melt for plotting
...
```{r phyla_out, echo=FALSE, warning=FALSE, cache=TRUE,fig.cap='Relative abundance of bacterial and archael phyla in soil, water and airborne dust lifted by wind tunnel treatment. Bacterial and archael phyla abundances were calculated individually per kingdom.\\label{fig:phyla}'}
...
```{r cyano_out, echo=FALSE, fig.height=4,warning=FALSE, cache=TRUE,fig.cap='Relative abundance of cyanobacterial genera in soil, water and airborne dust lifted by wind tunnel treatment. Abundances are relative wo the whole bacterial community.\\label{fig:cyano}'}
print(cyano.plot)
```
...
```{r rich_out, echo=FALSE,fig.width=4,fig.height=3,fig.cap='Chao1 richness estimate\\label{fig:rich}', cache=TRUE}
...
```{r nmds_out, echo=FALSE,fig.height=6,fig.cap='NMDS visualisation of bacterial community structure\\label{fig:nmds}', cache=TRUE}
...
```{r OTU_source_out, echo=FALSE,fig.height=6,fig.cap='Phylum abundance and OTU source in soil samples and water samples. Source indicates whether individual OTUs (97 % similarity) contributing to the phylum abundance were found in airborne dust from the dune flank, or in water and sediment in water channels. Only phyla present at > 1% of all reads are shown for clarity. Source designation is not indicative of the ultimate origin of OTUs. \\label{fig:source}', cache=TRUE}

#otu_source <- plot_bar(bac.rel.ea,x = "zone",fill = "source") + geom_bar(aes(color=source, fill=source), stat="identity", position="stack") + scale_y_continuous(name="relative abundance (%)") 
...
tax_table(bac.rel.ea.p)[waterborne_otus,"source"] <- "found in water channel"
...
water_otus_plot <- water_otus_plot + scale_y_continuous(name="relative abundance (%)") + scale_fill_brewer(palette = "Set1") + scale_color_brewer(palette = "Set1")
water_otus_plot <- water_otus_plot + facet_wrap(~Phylum, nrow=2) 

print(water_otus_plot + theme(legend.position="bottom"))
...
```{r deseq_out1, echo=FALSE, cache=TRUE}
...
```{r deseq_out2, echo=FALSE,fig.height=6,fig.cap='Relative abundance of bacterial phyla. Source indicates whether individual OTUs (97 % similarity) contributing to the phylum abundance were significantly more or less abundant in the wind tunnel filter samples compared to the whole soil. Only the 10 most abundant phyla are shown for clarity. Source designation is not indicative of the ultimate origin of OTUs.\\label{fig:airbornep}', cache=TRUE}
...
wind_otus_plot <- wind_otus_plot + scale_y_continuous(name="relative abundance (%)") + scale_fill_brewer(palette = "Set1") + scale_color_brewer(palette = "Set1")
wind_otus_plot <- wind_otus_plot + facet_wrap(~Phylum, nrow=2) 

print(wind_otus_plot + theme(legend.position="bottom"))
```


```{r deseq_out3, echo=FALSE,fig.height=4,fig.cap='Relative abundance of cyanobacterial genera. Source indicates whether individual OTUs (97 % similarity) contributing to the genus abundance were significantly more or less abundant in the wind tunnel filter samples compared to the whole soil. Only the 4 most abundant cyanobacterial genera are shown for clarity. Source designation is not indicative of the ultimate origin of OTUs.\\label{fig:airbornec}', cache=TRUE}
...
wind_otus_plot.c <- wind_otus_plot.c + scale_y_continuous(name="relative abundance (%)") + scale_fill_brewer(palette = "Set1") + scale_color_brewer(palette = "Set1")
wind_otus_plot.c <- wind_otus_plot.c + facet_wrap(~Genus, nrow=1)

print(wind_otus_plot.c + theme(legend.position="bottom"))
```
