...
# the position of individual points.
  ...
  print(
    ...
    geom_text(aes(x=nlevels(Class)/3,y=max(log2FoldChange)),label=inclab,hjust=1) +
    geom_text(aes(x=nlevels(Class)/3,y=min(log2FoldChange)),label=declab,hjust=1)   
)

  t <- paste(tbase," p <",alpha)

  print(
  ggplot(subset(sigtab_type,sigtab_type$padj<alpha)) + ggtitle(t) +
