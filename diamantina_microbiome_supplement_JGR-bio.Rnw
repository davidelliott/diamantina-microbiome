...
\supportinginfo{Landscape Stability in Drylands Influenced by Dispersal Strategy of Soil Bacteria}
...
\begin{enumerate}
\item Captions for large Tables S1 to S3 (if larger than 1 page, upload as separate excel file)
\end{enumerate}
...
\section*{Large Table S1.} %Elliott_Diamantina_Microbiome_Table_S1[phyla].csv

relative abundance of all prokaryotic phyla. 

\section*{Large Table S2.} %Elliott_Diamantina_Microbiome_Table_S2[cyanobacteria].csv

relative abundance of cyanobacteria

\section*{Large Table S3.} %Elliott_Diamantina_Microbiome_Table_S3[wind].csv

Differential mobilisation of OTUs by wind tunnel treatment

\section*{Large Table S4.} %Elliott_Diamantina_Microbiome_Table_S4[otus].csv

OTU abundance in each sampling zone (pc) - full version of excerpt tables presentd in manuscript. Sig column indicates if the OTU was significantly more abundant in soil or wind tunnel filter after running the wind tunnel on dune flanks. 
