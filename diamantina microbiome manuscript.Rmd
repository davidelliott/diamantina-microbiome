...
```{r locations, echo=FALSE, message=FALSE, eval=TRUE, cache=TRUE}
library(ggmap)
library(sp)
...
```{r map2,echo=FALSE, eval=TRUE, cache=TRUE}
...
\subsection{Sampling}
Sampled areas were 10x10 cm and were selected to represent the diversity of biocrust in the area (Figure \ref{fig:photo-sampling}). The sampled depth was dictated by coherence of the biocrust. The area was cut out with a sterile scalpel and the crust was lifted off the looser soil beneath. In areas where differing crust types were noticeable the sampled areas were selected to have both types where applicable (e.g. biological/physical, light/dark). In recently disturbed areas remnants of pre-disturbance crust were selected (2x dune sites).
...
In addition to the natural crust samples a wind tunnel was run on the dune flank zones within 3 m of where the natural sampling was done. The wind tunnel was run on the dune flanks because they are naturally eroded by saltation impact from the dune sand above when it is windy, therefore the potential as an inoculum source to surrounding areas is greater than the higher dune areas. A saltation source was not added to these experiments for the sake of simplicity, as there will be saltation occurring naturally within the wind tunnel anyway. At each site the wind tunnel was run three times on adjacent areas for 5 minutes at 10 m/s for 5 minutes\footnote{Helene collected data to determine the exact run conditions}. This is a moderate treatment intended to be realistic and to collect only the easily mobilised particles / organisms. 

\subsection{DNA sequencing}
DNA was extracted in the field within 24 hours of sample collection using a Mobio Powersoil DNA extraction kit. DNA sequencing targeted the prokaryotic 16S rRNA gene and was performed by the Centre for Genomic Research (CGR) NERC Facility at the University of Liverpool, using an Illumina MiSeq in a Paired end approach. Resultant FastQ files were processed using QIIME [@caporaso2010qiime] for clustering, OTU table generation, and taxonomy assignment. These results were analysed using R and the phyloseq package [@mcmurdie2013phyloseq] for R.

\section{Results}
This section may be divided by subheadings. Footnotes should not be used and have to be transferred into the main text.
DNA sequencing raw data, summary statistics, and methods description are available via the CGR LIMS at <http://cgr.liv.ac.uk/illum/LIMS10796_978f1f02f3c84a18/>

\subsection{Descriptive and exploratory analyses}
Blue-green platelet crusts were common but not the dominant surface cover on the claypan. They appear to have once been very widespread but have now been largely eroded away. Underneath them was a similar looking crust which is more brown (Figure \ref{fig:photo-sampling}), and this was not sampled in the present studt. In many places on the claypan there were nebkha on top of the blue-green platelet crusts and the platelets ran up the nebkha slightly, however the crusts on nebkha appeared distinct. Platelet crusts appeared to be more common near nebkha. 

In summary, three distinct surface layers were observed in the claypan:
\begin{enumerate}
\item deeper layer brown crust (not sampled)
\item platelet formation surface biological crust (sampled: P)
...
The microbial community structure of the soil surface varied markedly in relation to soil type (e.g. Figure \ref{fig:nmds}). The functional relevance of this is not clear, however this can be explored further as follows:
\begin{itemize}
\item Integrate / correlate experimental results with the microbial data
\item Use the literature to investigate possible functional roles for differentially abundant microbes
\item Perform future work specifically focussed on tackling questions relating to microbial function (for example with inclusion of enzyme activity assays and soil gas flux measurements)
\end{itemize}

\subsection{Dispersal and ecology of biocrust microbes}
The higher proportion of DNA sequence reads shared between dune and river compared to river and pan, may be explained by the fact that flooding of the pan is a rare event (last occurring x years previously) but mobilisation of dust from sand dunes is occurring continuously. Due to the dune being at higher elevation than all other locations, it might be expected to harbour fewer OTUs of aquatic origin, however this does not seem to be the case. In fact our data are not informative with respect to the direction of dispersal. This question could be tackled in future work by monitoring microbial populations over time and relating this to weather conditions and occurrence of flood events. I expect that co-occurrence between dune and river channel may be driven principally by wind dispersal (direction dune>river), whereas co-occurrence between river channel and claypan may be driven principally by flood events (direction river>claypan). Dispersal is one thing, but linking this to function at landscape scale is perhaps the biggest challenge.

I believe these data are quite unique and can yield a lot of useful results linking microbiology with fluvial and aeolian processes which has rarely been done before especially at the local level. The scope for speculating on microbial ecology is large, and the fact that microbes are differentially liberated from the soil surface by wind tunnel treatment provides an opportunity to investigate (with present data or in future work) whether microbes are specifically adapted in this regard or not. I tend to think that many of these will actually be adaptations, and one example is a \textit{Deinococcus} species which was found to be significantly more likely to become airborne than to stay in the soil during wind treatment. This genus is famous for its remarkable ability to withstand dessication and all kinds of radiation [@makarova2001genome]. It has been found in desert soils before [e.g. @rainey2005extensive] but according to the latest version of Bergey's manual [@whitman2015bergey] the natural habitat of this organism remains unknown. I believe our data would support the hypothesis that the habitat of this ancient lineage is in fact the biocrust top surface, and it is adapted for wind dispersal. With such ecology \textit{Deinococcus} species may serve a protective function to the biocrust, and can also ensure that they quickly colonise any newly emerging biocrust which will immediately benefit from such protection early in their development. In contrast to \textit{Deinococcus} species many OTUs were found to resist becoming airborne, and the relevance of this in terms of wind erosion, land management, and geo-engineering can all be explored. Such organisms (including most \textit{Cyanobacteria}) are less likely to be dispersed by the wind, and therefore in severely degraded areas biocrust may not re-form easily because the inoculum must come from a local source (e.g. vegetative growth or transport by an animal). Possible difficulties for biocrusts to regenerate after extreme sustained disturbance is something we suggested as a risk in @elliott2014niche, and seems to be supported by the present results.
